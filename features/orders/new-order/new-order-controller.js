...
export class NewOrderController {
  ...
  async init() {
    ...
    if (draftLoaded) {
      await this._restoreDraftFlow();
      console.log('restored')   // restoreDraftToTable will find DOM.orderTbody now
    } else {
  ...
  async _restoreDraftFlow() {
    const draft = this.stateManager.getState();
    console.log('[DEBUG] _restoreDraftFlow draft=', draft);
    console.log('[DEBUG] DOM.orderTbody=', this.DOM.orderTbody);
    const products = await fetchAllProducts(false);
    console.log('[DEBUG] products.length=', Array.isArray(products) ? products.length : products);
